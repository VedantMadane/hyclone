...
for package in "${array[@]}"; do
    ...
    hpkgResponseJson="$(curl --retry $CURL_RETRY_COUNT -Ls --request POST \
        --data '{"name":"'"$package"'","repositorySourceCode":"haikuports_'$HAIKU_ARCH'","versionType":"LATEST","naturalLanguageCode":"en"}' \
        --header 'Content-Type:application/json' "$HAIKU_DEPOT_BASE_URL")"
    hpkgDownloadUrl="$(echo "$hpkgResponseJson" | sed -n 's/^.*hpkgDownloadURL":"\([^"]*\)".*$/\1/p')"
    if [ -z "$hpkgDownloadUrl" ]; then
        echo "Failed to parse download URL for $package."
        echo "Response was:"
        echo "$hpkgResponseJson"
        exit 1
    fi
    hpkgVersion="$(echo "$hpkgDownloadUrl" | sed -n 's/^.*\/[^\/]*-\([^-]*\-[^-]*\)-[^-]*\.hpkg$/\1/p')"
    if [ -z "$hpkgVersion" ]; then
        echo "Failed to parse version for $package."
        echo "Download URL was: $hpkgDownloadUrl"
        exit 1
    fi
